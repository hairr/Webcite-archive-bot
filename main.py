...
class WebCiteArchiveBot(object):
    def __init__(self, username, password):
        ...
        site.plain_cite_template = '{{PlainCiteGeneral|url=%s|title=%s|' \
                                   'accessdate=%s|archiveurl=%s|archivedate=%s}}'
    ...
    def add_in_template(self, text, url, archiveurl):
        ...
        for template in code.filter_templates():
            ...
            if lowercase in self.cite_templates:
                ...
                if not template.has_param('accessdate'):
                    ...
                    access_date_value = template.get('accessdate').value
                    access_date_value.replace(access_date_value,
                    self.format_date())
        ...
        if str(original) != str(code):
            return str(code)
        regex_code = r'<ref(.*?)>\w?\[\w?%s\w?(.*?)\]\w?</ref>'
        regex = re.match(regex_code % re.escape(url),
